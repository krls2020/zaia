...
if [ $# -eq 0 ]; then
  zaia_uri="https://github.com/krls2020/zaia/releases/latest/download/zaia-${target}"
else
  zaia_uri="https://github.com/krls2020/zaia/releases/download/${1}/zaia-${target}"
fi

bin_dir="$HOME/.local/bin"
bin_path="$bin_dir/zaia"
bin_dir_existed=1
...
curl --fail --location --progress-bar --output "$bin_path" "$zaia_uri"
chmod +x "$bin_path"

echo
echo "ZAIA was installed successfully to '$bin_path'"

if command -v zaia >/dev/null; then
  echo "Run 'zaia --help' to get started"
